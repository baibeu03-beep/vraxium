...
export async function GET(request: NextRequest) {
  try {
    ...
    if (seasonHistoriesResult.data?.length) {
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      console.log('[Profile API] season_id values from histories:', (seasonHistoriesResult.data as any[]).map((h: any) => h.season_id));
    }
    console.log('[Profile API] seasonsByUuidMap size:', seasonsByUuidMap.size, 'sample keys:', Array.from(seasonsByUuidMap.keys()).slice(0, 3));
    console.log('[Profile API] season_definitions count:', allSeasonsRaw.length);

    // user_season_histories raw 데이터에 seasons 객체 attach (client-side merge).
    // user_season_histories.season_id 는 seasons 테이블의 UUID 를 참조하므로
    // seasonsByUuidMap (seasons.id → season data) 으로 매핑한다.
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    ...
    if (finalSeasonHistories.length > 0 && supabaseAdmin) {
      ...
      // seasons 테이블 (UUID 기반, 실제 start_date/end_date 보유) 에서 필터
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      const currentSeasons = seasonsTableData.filter((s: any) =>
        s.start_date && s.end_date && s.start_date <= today && s.end_date >= today && !s.name?.toLowerCase().includes('break')
      );
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
