...
// Phalanx 전용 cluster-4-card 변형. 원본은 현재/최신 주차로 자동 redirect 한다.
// PX 변형도 동일 동작이되, redirect 목적지를 /cluster-4-card-px 로 한정해
// PX 라우트 내부에 머무르게 한다.
import { useState, useEffect } from "react";
import { useRouter } from "next/navigation";
import { supabase } from "@/lib/supabase";

const Cluster4CardPxPage = () => {
  ...
  useEffect(() => {
    ...
    if (demoOn) {
      router.replace(withQs("/cluster-4-card-planning/dw-01"));
      return;
    }
        ...
        if (error || !currentWeek) {
          ...
          if (latestWeek) {
            ...
            if (!seasonName.toLowerCase().includes("break")) {
              router.replace(withQs(`/cluster-4-card-planning/${latestWeek.id}`));
              return;
            }
          }
        } else {
          ...
          if (!seasonName.toLowerCase().includes("break")) {
            router.replace(withQs(`/cluster-4-card-planning/${currentWeek.id}`));
            return;
          }
        }
...
export default Cluster4CardPxPage;
