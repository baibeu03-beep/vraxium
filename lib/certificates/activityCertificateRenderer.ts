...
/**
 * 렌더된 PNG 를 1페이지 PDF 로 감싼다. 재렌더가 없으므로 PNG 와 텍스트 위치가 100% 동일.
 *
 * ⚠️ pdf-lib 의 페이지 단위는 pt(1/72inch)다. 픽셀 값을 그대로 쓰면 비현실적으로 큰
 *    페이지가 나온다. pdfDpi 로 환산한다. 이미지는 원본 해상도 그대로 임베드되고
 *    페이지 박스에 꽉 차게 그려지므로 비율 유지 · 잘림 없음 · 여백 0 이다.
 */
export async function renderActivityCertificatePdf(
  rendered: RenderedCertificate,
  issueDateIso: string,
): Promise<Buffer> {
  const dpi = ACTIVITY_CERTIFICATE_TEMPLATE.pdfDpi;
  const pageWidth = (rendered.width / dpi) * 72;
  const pageHeight = (rendered.height / dpi) * 72;
